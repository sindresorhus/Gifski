//
//  PreBakedFrames.swift
//  Gifski
//
//  Created by Michael Mulet on 4/25/25.
//
...
struct PreBakedFrames {
	private let frames: [Int: MTLTexture]
	...
	init(_ imageSource: CGImageSource, settings: SettingsForFullPreview) async throws {
		...
		let dict = try await allFrames.asyncMap { frameIndex in
			...
			return (frameIndex, try await PreviewRenderer.convertToTexture(cgImage: image))
		}
		self.frames = Dictionary(uniqueKeysWithValues: dict)
	}

	func getPreBakedFrame(forTime time: PreviewVideoCompositor.OriginalCompositionTime) -> MTLTexture? {
	...
	struct Offsets: Equatable {
		...
		init(settings: SettingsForFullPreview){
			...
			self.init(
				...
				originalTrackTimeRange: CMTimeRange(start: .zero, duration: .init(seconds: settings.assetDuration, preferredTimescale: .video))
			)
		}

		init(fullPreviewStartTime: CMTime, fullPreviewEndTime: CMTime, originalTrackTimeRange: CMTimeRange) {
			start = fullPreviewStartTime <= originalTrackTimeRange.start ? CMTime(seconds: PreBakedFrames.bugFixOffset, preferredTimescale: .video) : .zero
			end = fullPreviewEndTime >= originalTrackTimeRange.end ? CMTime(seconds: PreBakedFrames.bugFixOffset, preferredTimescale: .video) : .zero
		}
	}
}
